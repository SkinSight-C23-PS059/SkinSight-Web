...
myAccountLink.addEventListener('click', (event) => {
  ...
  const isLoggedIn = false; // Ganti dengan logika pengguna yang telah login

  if (!isLoggedIn) {
    Swal.fire({
      title: 'Oops!',
      text: 'Maaf, Anda belum memiliki masuk. Silakan login terlebih dahulu',
      icon: 'error',
      ...
      if (result.isConfirmed) {
        // Alihkan ke halaman login.html
        window.location.href = 'login.html';
      }
    });
  } else {
    // Tambahkan logika yang diperlukan jika pengguna sudah login
    // Contoh: Alihkan ke halaman profil pengguna
...
function saveToBookmark() {
  // Periksa apakah pengguna sudah login (misalnya dengan memeriksa token otentikasi)
  ...
  if (!token) {
    // Jika pengguna belum login, tampilkan SweetAlert2 untuk informasi
    Swal.fire({
      icon: 'warning',
      title: 'Login Required',
      text: 'You need to log in to save this bookmark.',
      showCancelButton: true,
      confirmButtonText: 'Yes',
      cancelButtonText: 'No',
    }).then((result) => {
      if (result.isConfirmed) {
        // Jika pengguna memilih "Ya" (Yes), alihkan ke halaman login
